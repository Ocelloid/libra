...
const SeeInvitesModal: React.FC<{
  ...
  const { mutate: membershipMutation } =
    api.Team.respondToInvitation.useMutation();

  const { refetch } = api.Team.getAllInvitations.useQuery(undefined, {
  ...
  const handleResponse = (inviteId: string, accepted: boolean) => {
    ...
    membershipMutation(
      { membershipId: inviteId, status: accepted ? "accepted" : "declined" },
      {
        async onSuccess() {
          await refetch();
          onRefetch();
  ...
  const handleConfirm = (
    ...
    setInvites(
      invites.map((invite: ConfirmedMembership) => {
        if (invite.id === inviteId) {
        ...
        {() => (
          <>
            ...
            <ModalBody className="pt-0">
              {invites.map((invite) => (
                <div key={invite.id} className="flex flex-1 flex-row gap-1">
                  <div className="flex flex-1 flex-col truncate py-2">
                    {invite.team?.title}
                  </div>
