...
export const TeamRouter = createTRPCRouter({
    ...
    createTeam: protectedProcedure
        .input(z.object({ title: z.string(), description: z.string(), userEmails: z.string().array() }))
        .mutation(async ({ ctx, input }) => {
            const {db, session} = ctx;
    ...
    updateTeam: protectedProcedure
        .input(z.object({ teamId: z.string(), title: z.string(), description: z.string(), userEmails: z.string().array() }))
        .mutation(async ({ ctx, input }) => {
            const {db, session} = ctx;
            ...
            const oldUserEmails = newTeam?.memberships.map(member => member.user.email ?? "");

            const removedMembers = oldUserEmails.filter(email => !userEmails.includes(email));            
            ...
            const addedMembers = userEmails.filter(email => !oldUserEmails.includes(email));
            if (addedMembers.length > 0) {
                ...
                const checkUsers = checkInvitations.map(invitation => invitation.user.email ?? "")
                const newInvitations = addedMembers.filter(email => !checkUsers.includes(email))
                ...
                const newMemberships = newReceivers.map(receiver => {return {
    ...
    sendInvitation: protectedProcedure
        ...
        .mutation(async ({ctx, input}) => {
            ...
            if (receiver) {
                ...
                if (checkInvitation) { // changing membership status
    ...
    countAllInvitations: protectedProcedure
        .query(async ({ ctx }) => {
            const {db, session} = ctx;
